...
class Organizer(multiprocessing.Process):
    def __init__(self, urls, log_file, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.writer = multiprocessing.Queue(maxsize=2)
        self.processes = []
        self.urls = urls
    ...
    def run(self):
        for url in self.urls:
            if url.find("pikabu") != -1:
                process = Pikabu_Downloader(url, self.writer)
            elif url.find("reddit") != -1:
                process = Reddit_Downloader(url, self.writer)
            self.processes.append(process)
        for process in self.processes:
            process.start()
        while True:
            try:
                log = self.writer.get(timeout=1)
                Downloader._log_writer(file = self.log_file, log = log)
            except queue.Empty:
...
class Pikabu_Downloader(Downloader):
    def __init__(self, url, writer, *args, **kwargs):
    ...
    def run(self):
        ...
        self.writer.put(self.log_list[0])
...
class PageSizer(multiprocessing.Process):
    ...
    def run(self):
        ...
        if self.go_ahead:
            ...
            sizers = [PageSizer(url = url, collector=collector, go_ahead=False) for url in parser.links]
            for sizer in sizers:
                sizer.start()
            for sizer in sizers:
                sizer.join()
...
@time_duration
def download():
    organizer_pi = Organizer(pikabu_urls, "log_pikabu.txt")
    organizer_re = Organizer(reddit_urls, "log_reddit.txt")
    for organizer in [organizer_re, organizer_pi]:
        organizer.start()
        organizer.join()
