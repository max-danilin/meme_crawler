...
class Organizer(Thread):
    def __init__(self, urls, log_file, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.writer = queue.Queue(maxsize=2)
        self.threads = []
        self.urls = urls
    ...
    def add_thread(self, url):
        if url.find("pikabu") != -1:
            thread = Pikabu_Downloader(url , self.writer)
        elif url.find("reddit") != -1:
            thread = Reddit_Downloader(url, self.writer)
...
def download():
    ...
    for organizer in [organizer_re, organizer_pi]:
        for url in organizer.urls:
            organizer.add_thread(url)
    
        organizer.start()
        organizer.join()
        
    # reddit_dls = [Reddit_Downloader(url) for url in reddit_urls]
